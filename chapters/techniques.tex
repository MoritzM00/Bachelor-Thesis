...
und die für die Visualisierung sehr verbreitete Methode \newterm{t-distributed stochastic neighbor embedding} (t-SNE) in \subsecref{ch:MethodenDerDimRed:traditionell:t-SNE}.
...
Dazu gehen wir wieder in \subsecref{ch:MethodenDerDimRed:traditionell:kPCA:Grundidee} auf die Grundidee dieser Methode ein und befassen uns dann in \subsecref{ch:MethodenDerDimRed:traditionell:kPCA:AnwendungKernelTrick} kurz mit der konkreten Vorgehensweise bei der Kernel PCA.

\subsubsection{Grundidee}
\label{ch:MethodenDerDimRed:traditionell:kPCA:Grundidee}

Die Grundidee der Kernel PCA ist es, die Daten mithilfe einer Abbildung $\bm{\phi}$ in einen Raum $\mathcal{H}$ abzubilden, der nichtlinear mit dem Merkmalsraum\unsure{vlt hier einfach $\real^D$ anstelle von $\mathcal{X}$ benutzen um Verwirrung zu vermeiden. Vlt muss ich dafür auch die Begriffe eindeutiger definieren (feature space und x-Space)} $\mathcal{X}$ von $\rvect{x}$ verbunden ist. Die Motivation dahinter ist, dass die Daten in $\mathcal{X}$ auf einer \textit{nichtlinearen} Mannigfaltigkeit liegen können, während sie in $\mathcal{H}$ auf einer \textit{linearen} Mannigfaltigkeit liegen und damit für lineare Dimensionsreduktionsmethoden wie PCA geeignet sind. Nach der Transformation wird also
...
Nun können die Eigenvektoren und Eigenwerte von $\mat{K}$ bestimmt und damit die Projektion auf die niedrigdimensionale Repräsentation $\rvect{y}$ berechnet werden.\rewrite{Diesen Abschnitt (3.1.2.2) muss ich nochmal ganz umschreiben}
...
Um eine sinnvolle Repräsentation $\rvect{y}$ zu erhalten, muss der Autoencoder oftmals regularisiert, das heißt eingeschränkt, werden. Ein Beispiel hierfür ist der in \figref{fig:5-layer-Autoencoder} gezeigte sogenannte \newterm{unterbestimmte} Autoencoder. Das bedeutet, dass die Bottleneck-Schicht kleiner ist, als die Dimension des Inputvektors.\addref

Neben dieser intuitiven Regularisierungsmethode gibt es noch viele weitere Methoden, wie zum Beispiel der \newterm{Sparse} Autoencoder oder der Variational Autoencoder (\subsecref{ch:MethodenDerDimRed:modern:VAE}).
