...
Mathematisch gesehen versucht der Autoencoder zwei Funktionen zu lernen. Zum einen den sogenannten \newterm{Encoder} $f: \real^D \rightarrow \real^k$, der der linken Hälfte des Autoencoders entspricht und zum anderen den \newterm{Decoder} $g: \real^k \rightarrow \real^D$, der die rechte Hälfte des Autoencoders widerspiegelt ($k < D$). Das Ziel des Autoencoders ist es, dass der Outputvektor $\tilde{\rvect{x}}$ ungefähr dem Inputvektor entspricht und dabei eine sinnvolle Repräsentation im Bottleneck gelernt wird:
\begin{equation}
