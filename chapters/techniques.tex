...
\idea{Weight Decay als einfachste Methode?}
...
$\vect{y}$ sind ungleich Null. Dazu wird ein Bestrafungsterm $\Omega(\vect{y})$ auf den Output des
Encoders $\vect{y} = f(\vect{x})$ zur Fehlerfunktion hinzugefügt, das heißt
\begin{equation}
	L(\vect{x}, \vect{\estNormal{x}}) = L_{rec} + \lambda \Omega(\vect{y})
\end{equation} \Parencite[505]{Goodfellow.2016}, über den Parameter $\lambda$ die Stärke der Regularisierung kontrolliert wird. Für das Lernen von Mannigfaltigkeiten besonders interessant ist jedoch der
...
mit der von PCA wie folgt:\unsure{was meine ich mit "Lösung" des linearen Autoencoders? Genauer
	beschreiben}
\begin{enumerate}
...
Fehlerfunktion des Autoencoders um einen zusätzlichen Regularisierungsterm erweitert. Die Idee ist
es, die \textit{Sensitivität} des Autoencoders für den Input zu bestrafen. Das bedeutet, dass bei
kleiner Änderung des Inputs $\vect{x}$ die Kodierung $f(\vect{x})$ sich ebenfalls nur minimal
ändern soll. Dies entspricht einer kleinen ersten Ableitung und kann über die (quadrierte)
Frobeniusnorm der Jacobi-Matrix $\mat{J}$ des Encoders $f$ erzielt werden \parencite[2]{Rifai.2011}:
\begin{equation}
	\label{eq:CAE-Regularisierung}
	\norm{\mat{J}_f(\vect{x})}_F^2 = \sum_{ij} \left( \frac{\partial f_i(x)}{\partial x_j} \right)^2
\end{equation}