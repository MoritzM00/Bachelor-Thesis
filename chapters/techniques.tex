...
zum Beispiel mit der Hauptkomponentenanalyse (\subsecref{ch:MethodenDerDimRed:statistisch:PCA}) der
...
\secref{ch:MethodenDerDimRed:modern} eingehend behandelt.
...
die nichtlineare Erweiterung der Hauptkomponentenanalyse, die \newterm{Kernel Principal Component
	Analysis} (Kernel PCA) (\subsecref{ch:MethodenDerDimRed:statistisch:kPCA}). Außerdem wird das
\newterm{Locally Linear Embedding} (LLE) in \subsecref{ch:MethodenDerDimRed:statistisch:LLE} als
...
sowie die mathematische Herleitung der sogenannten \newterm{Hauptkomponenten} (Unterabschnitte
...
Um das zu erreichen, wird sich am sogenannten \newterm{Kernel Trick} bedient, der schon erfolgreich auf \textit{Support Vector Machines} \parencite{Boser.1992} angewandt wurde. Dazu wird in
\subsecref{ch:MethodenDerDimRed:statistisch:kPCA:Grundidee} die Grundidee dieser Methode erläutert
und in \subsecref{ch:MethodenDerDimRed:statistisch:kPCA:Vorgehensweise} wird sich mit der konkreten
...
Die Performance der Kernel PCA hängt entscheidend von der Wahl des Kernels ab.\rewrite{Das tut sie
	laut Fabian nicht. Entscheidender ist die Wahl der Parameters des Kernels!} Leider gibt es keine
Möglichkeit, vorab den besten Kernel für ein spezifisches Problem auszuwählen. Dies ist die größte
Schwachstelle der Kernel PCA und ein Grund, wieso sich die Kernel PCA nicht so stark in der
Dimensionsreduktion durchsetzen konnte wie die klassische Hauptkomponentenanalyse. Allgemein umgeht
man das Problem der manuellen Auswahl eines Kernels für Kernel-Methoden mittels Kreuz-Validierung
(engl. \textit{Cross-Validation} (CV)). Dabei wird ein Qualitätskriterium festgelegt und die
Methode mit einer vorab definierten Auswahl an Kernel-Funktionen auf einer Teilmenge des
Trainingsdatensatzes trainiert und evaluiert. Die Parameter der Methode, die hinsichtlich des
Qualitätskriteriums am besten abgeschnitten hat, werden dann für das eigentliche Modell übernommen
und auf dem größeren Trainingsdatensatz trainiert. Dies funktioniert für Klassifikatoren und
Regressoren wie z.B. die Support Vector Machine gut, da die Wahl der Qualitätskriterien klar ist.
Für die Dimensionsreduktion ist die Wahl des Qualitätskriteriums nicht so trivial, wie wir später
in \subsecref{ch:Vergleich:sec:Methodik:subsec:Qualitaetskriterien} sehen werden.

In der Praxis häufig anzutreffen sind Gaußsche Kernel (auch: Radiale Basisfunktionen (RBF))
\begin{equation}
	\kappa(\vect{x}_i, \vect{x}_j) = \exp \left(- \frac{\norm{ \vect{x}_i - \vect{x}_j}^2}{2\sigma}\right) \, ,
\end{equation}
wobei der Parameter $\sigma > 0$ die \enquote{Breite} des Kernels bestimmt \parencite[296]{ShaweTaylor.2011}. Außerdem sind polynomiale Funktionen \parencite[292]{ShaweTaylor.2011}
\begin{equation}
	\kappa(\vect{x}_i, \vect{x}_j) = (\vect{x}_i \cdot \vect{x}_j)^p
\end{equation}
des Grades $p$ eine häufige Wahl des Kernels.
%% ==============================
...
(\subsecref{ch:MethodenDerDimRed:ML:AE}) und eine Modifizierung in Form des \newterm{Contractive
	Autoencoder} (CAE) (\subsecref{ch:MethodenDerDimRed:ML:CAE}) vorgestellt.
...
\figref{fig:5-layer-Autoencoder} gezeigte symmetrische Architektur. \rewrite{hier eine allgemeinere
	Darstellung wählen}
\begin{figure}[ht]
...
immer noch weit weg auf der Mannigfaltigkeit liegen \parencite[521]{Goodfellow.2016}. Dazu wird wieder in \subsecref{ch:MethodenDerDimRed:CAE:Grundidee}
