...
wird sich am sogenannten \newterm{Kernel Trick} bedient, der schon erfolgreich auf \textit{Support
...
\end{align}
definiert ist \parencite[68]{ShaweTaylor.2011}. Die Eigenwertszerlegung der zentrierten Kernel Matrix\footnote{Wir
	hatten in der klassischen Hauptkomponentenanalyse angenommen, dass die ursprünglichen Daten
	zentriert sind. Analog wollen wir, dass die Daten im Merkmalsraum $\mathcal{H}$ zentriert sind.
	Dies können wir implizit durch eine Transformation der Kernel-Matrix erreichen. Dieser Schritt ist
	jedoch nur für eine Vereinfachung der Berechnungen (der Kovarianzmatrix) notwendig, nicht aber für
	die Gültigkeit des Verfahrens. Die Zentrierung kann mittels
	\begin{equation}
		\widetilde{\mat{K}} = \mat{K} - \frac{1}{n} \ones \tr{\ones} \mat{K} - \frac{1}{n} \mat{K} \ones \tr{\ones} + \frac{1}{n^2} (\tr{\ones} \mat{K} \ones) \ones \tr{\ones}
	\end{equation}
	durchgeführt werden \parencite[131]{ShaweTaylor.2011}, wobei $\ones = \tr{(1, \ldots, 1)}$ der Vektor bestehend aus Einsen
	ist.} liefert mit einer entsprechenden Skalierung die Eigenvektoren der Kovarianzmatrix $\mat{C}$.
Konkret wird die zentrierte $n \times n$ Kernel Matrix in ihre Eigenvektoren $\vect{v_i}$ und
Eigenwerte $\lambda_i$ mit $i = 1, \ldots, n$ zerlegt, womit über den Zusammenhang
\begin{equation}
...
\begin{equation}
	\vect{y}_i = \Big( \sum_{j=1}^n \vect{a}_j^{(l)} \kappa(\vect{x}_i, \vect{x}_j) \Big)_{l=1}^d
\end{equation}
...
Gittersuche und kombiniert dies mit Kreuzvalidierung über. Beispielsweise wird ein
