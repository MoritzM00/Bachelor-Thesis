...
\textcites{Baldi.1989}{Bourlard.1988} haben gezeigt, dass die Kodierung $\vect{y}$ eines linearen Autoencoders äquivalent -- aber nicht identisch -- zu den Hauptkomponenten einer PCA (siehe \secref{ch:MethodenDerDimRed:statistisch:PCA}) ist: Die Gewichte des Autoencoders spannen den gleichen Unterraum auf wie die Eigenvektoren der Kovarianzmatrix der Daten. Dies gilt sogar dann, wenn in einem dreischichtiger Autoencoder nichtlineare Aktivierungsfunktionen verwendet werden \parencite[291, 293]{Bourlard.1988}. Jedoch unterscheidet sich die Lösung eines linearen Autoencoders
...
\end{enumerate}
\parencite[3]{Plaut.2018}. Betrachtet man beispielsweise einen einfachen linearen Autoencoder mit drei
Schichten, so kann der Encoder als
\begin{equation}
...
\end{equation}
formuliert werden, wobei $\mat{W}_i$ die Gewichtsmatrizen und $\vect{b}_i$ die Bias-Vektoren des Autoencoders sind ($i = 1, 2$). Insgesamt kann der Output direkt wie folgt ausgedrückt werden:
\begin{equation}
	\estNormal{\vect{x}} = g(f(\vect{x})) = \mat{W}_2(\mat{W}_1 \vect{x} + \vect{b_1}) + \vect{b}_2
\end{equation}\unsure{passen die Dimensionen der Matrizen?}
