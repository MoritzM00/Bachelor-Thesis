...
Fällen werden jedoch \textit{Sigmoid}-Aktivierungsfunktionen eingesetzt \addref, um die
Flexibilität der Architektur auszunutzen.
...
%\textcite{Plaut.2018}
\addref haben gezeigt, dass die Repräsentation $\rvect{y}$ eines linearen Autoencoders äquivalent, aber nicht identisch, zu den Hauptkomponenten einer PCA (siehe \secref{ch:MethodenDerDimRed:statistisch:PCA}) ist. Betrachtet man beispielsweise einen einfachen linearen Autoencoder mit drei Schichten, kann der Encoder als
\begin{equation}
...
\end{equation}
formuliert werden, wobei $\mat{W}_i$ die Gewichtsmatrizen und $\vect{b}_i$ die Bias-Vektoren des Autoencoders sind ($i = 1, 2$).
