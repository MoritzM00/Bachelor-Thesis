...
\subsubsection{Die Kernel-Funktion}
\label{ch:MethodenDerDimRed:statistisch:kPCA:KernelFunktion}

Die Kernel-Funktion ist definiert als das Skalarprodukt im Merkmalsraum \parencite[34]{ShaweTaylor.2011}:
\begin{equation}
...
\end{equation}
wobei $\bm{\phi}$ hier elementweise angewandt wird. Um nun die Hauptkomponentenanalyse durchführen zu können, müssten die Eigenvektoren dieser Matrix bestimmt werden. Dies ist jedoch nicht möglich, da die Abbildung $\bm{\phi}$ unbekannt ist. Wie sich herausstellt ist diese Matrix aber sehr ähnlich zur sogenannten \textit{Kernel Matrix} $\mat{K}$, welche über die Kernel-Funktion $\kappa$ als
\begin{equation}
	\label{eq:kPCA:KernelMatrix}
	\begin{aligned}
		\mat{K} & = \left( \kappa(\vect{x}_i, \vect{x}_j) \right)_{i,j = 1,\ldots,n} \\
		        & = \bm{\phi}(\mat{X}) \tr{\bm{\phi}(\mat{X})}                       \\
	\end{aligned}
\end{equation}
definiert ist \parencite[68]{ShaweTaylor.2011}. Die Eigenwertszerlegung der zentrierten Kernel Matrix liefert die
Eigenvektoren der Kovarianzmatrix $\mat{C}$.
