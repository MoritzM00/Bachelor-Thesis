...
Nachfolgend werden in \secref{ch:Vergleich:sec:Resultate} die Resultate des Vergleichs der zwei
Gruppen diskutiert und abschließend wird in \secref{ch:Vergleich:sec:Resultate:PCA_AE} der
...
Der Vergleich wird auf insgesamt sieben Datensätzen durchgeführt, wobei zwei künstliche und fünf
natürliche Datensätze verwendet werden. Diese werden in
\secref{ch:Vergleich:sec:VerwendeteDatensaetze} genauer vorgestellt. Auf Bilddatensätzen wird
zusätzlich der bei der Wahl der Architektur in \secref{ch:MethodenDerDimRed:ML:AE} erwähnte
...
\subsecref{ch:Vergleich:sec:Methodik:subsec:SchaetzenDerIntrinsischenDim} erklärt. Der Vergleich
...
Zu den künstlichen Datensätzen gehören die weitverbreitete \textit{Swiss Roll} und der \textit{Twin Peaks} Datensatz, die in \figref{fig:ArtificialDatasets} dargestellt sind.
\begin{figure}[htb]
	\begin{center}
...
damit \num{2304}-dimensional. Beispiele der Bilddatensätze sind in \figref{fig:Dataset_samples} zu
finden.
\begin{figure}[hbt]
	\begin{center}
...
\end{figure}
Beide Abbildungen zeigen ein ähnliches Ergebnis: Locally Linear Embedding kann mit der lokalen Erhaltung der Struktur hohe Werte auf allen drei Qualitätskriterien erzielen. Insbesondere hinsichtlich des LCMC kann LLE die anderen Methoden deutlich outperformen.
...
Auf Datensätzen mit einer geringen Anzahl an Stichproben können Probleme bei der Konvergenz der iterativen Optimierung eines Autoencoders auftreten. Dies wird durch die Performance der Autoencoder auf dem ORL-Datensatz ersichtlich (siehe \figref{fig:OlivettiFacesMetrics}). Dies ist möglicherweise ein Nachteil der iterativen Optimierung einer komplexeren Zielfunktion gegenüber der Lösung einer simpleren Zielfunktion in geschlossener Form.
...
Das manuelle Hyperparameter-Tuning wurde in den Laufzeiten nicht berücksichtigt. Dies ist ein
...
Interessant ist außerdem, wie gut die Dimensionsreduktionsmethoden ein Bild aus der latenten
...
\figref{fig:MNIST-reconstructions} dargestellt. Die erste Zeile besteht aus den Originalbildern als
Referenz.
\begin{figure}[ht]
	\centering
...
\textcite{Bourlard.1988} haben gezeigt, dass die Kodierung $\vect{y} = f(\vect{x})$ eines linearen Autoencoders äquivalent -- aber nicht identisch -- zu den Hauptkomponenten $\tr{\mat{A}}\vect{x}$ einer Principal Component Analysis ist. Konkret wurden dabei dreischichtige Autoencoder betrachtet, d.h. die Gewichte des Encoders sind eine Matrix $\mat{W}_1 \in \real^{d \times D}$ und die Gewichte des
...
\secref{ch:MethodenDerDimRed:ML:AE}). Die Regularisierung der Gewichte bezweckt, dass die
...
Der eben beschriebene Zusammenhang wird im Folgenden zur Verdeutlichung visualisiert. Dazu werden
...
Um das Vorgehen von \textcite{Plaut.2018} zu überprüfen, sind in \figref{fig:Kovarianzmatrizen}
\begin{figure}[ht]
	\centering
...
Um nun die gefundenen Lösungen der Principal Component Analysis und Autoencoder genauer betrachten
...
den MNIST-Datensatz berechnet und in \figref{fig:Autoencoder-Nichtlinearitaet} dargestellt. Für die
...
Zusammenfassend kann festgehalten werden, dass ein dreischichtiger Autoencoder mit linearen
