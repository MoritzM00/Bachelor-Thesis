...
ist definiert als die Menge der Indizes\unsure{Nachbarschaft als Menge der Indizes oder Vektoren
	definieren? je nachdem muss ich dann auch unten die Mengen U und V anpassen, so wie es jetzt gerade
	ist, ist es falsch} der $K$-nächsten Punkte zu $\vect{x}_i$ ($i = 1, \ldots, n$). Analog kann die
$K$-Nachbarschaft $\widetilde{\set{M}}_i(K)$ des dazugehörigen niedrigdimensionalen Punktes
...
\begin{gather}
	\set{U}_i(K) =  \left\{ j\, |\, \vect{x}_j \notin \set{M}_i(K) \land \vect{y}_j \in \widetilde{\set{M}}_i(K) \right\} \\
	\set{V}_i(K) =  \left\{ j\, |\, \vect{x}_j \in \set{M}_i(K) \land \vect{y}_j \notin \widetilde{\set{M}}_i(K) \right\}
\end{gather}
