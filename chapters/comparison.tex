...
Parameter. Die Anzahl Hauptkomponenten entspricht der intrinsischen Dimension, welche vom Datensatz
abhängt und für alle Methoden identisch ist. Dies trifft analog auf Kernel PCA und auf Locally
Linear Embedding zu. Für Autoencoder bestimmt die intrinsische Dimension die Größe der
...
Architektur zu bestimmen. Die wichtigsten Parameter sind die Anzahl der Schichten und die Wahl der
Aktivierungsfunktionen. Daneben gibt es noch viele weitere Freiheitsgrade für das Trainieren des
Autoencoders, welche teilweise vom Datensatz abhängen. Diese wurden für alle Autoencoder nahezu
identisch gewählt und werden in Appendix~\ref{ch:Appendix:Architektur-Details} genauer erläutert.
Für beide künstlichen Datensätze und für den Olivetti Face Datensatz wird ein dreischichtiger
...
Sigmoid-Aktivierungsfunktionen im Encoder und Decoder. Auf Bilddatensätze wurden zusätzlich
Convolutional Autoencoder trainiert, deren Architekturen an \textcite[14]{Ghosh.2019} angelehnt
sind und ebenfalls in Appendix~\ref{ch:Appendix:Architektur-Details} spezifiziert sind.
...
\begin{table}[]
	...
	\begin{tabular}{@{}cc@{}}
		...
		PCA              & --                                                   \\
		Kernel PCA       & RBF-Kernel, $\gamma=\sfrac{1}{D}$                    \\
		LLE              & $K=10$, $\epsilon=1\mathrm{e}{-3}$                   \\
		Autoencoder      & siehe Appendix~\ref{ch:Appendix:Architektur-Details} \\
		Contractive AE   & siehe Appendix~\ref{ch:Appendix:Architektur-Details} \\
		Convolutional AE & siehe Appendix~\ref{ch:Appendix:Architektur-Details} \\
		\bottomrule
	\end{tabular}
	\caption{Übersicht über die verwendeten Parameter}
...
wird die Hauptkomponentenanalyse und unterschiedliche Modelle des Autoencoder auf dem FER-2013
Datensatz trainiert. In \figref{fig:Gewichtsvergleich} sind die Gewichtsmatrizen von PCA, einem
linearen Autoencoder und einem Autoencoder mit Sigmoid-Aktivierungsfunktion im Encoder abgebildet.
Wie zu erkennen ist, sind die Gewichte von des linearen Autoencoders bis auf das Vorzeichen sehr
ähnlich zu der Ladungsmatrix von PCA.

\begin{figure}[ht]
	\centering
	\input{\figures/weights-comparison.pgf}
	\caption[Gewichtsmatrizen von ausgewählten Methoden]{(a) PCA (b) Linearer dreischichtiger Autoencoder (c) Nichtlinearer dreischichtiger Autoencoder, wobei der der Encoder eine Sigmoid-Aktivierungsfunktion einsetzt und der Decoder linear ist. Die Gewichte von PCA und dem linearen Autoencoder sind bis auf die Vorzeichen (erkennbar am invertierten Grauwert) sehr ähnlich. (Eigene Darstellung)}
...
\begin{figure}[ht]
	...
	\caption[Korrelationsmatrizen]{Korrelationsmatrizen von ausgewählten Methoden: (a) PCA (b) Linearer dreischichtiger Autoencoder (c) Nichtlinearer dreischichtiger Autoencoder, wobei der der Encoder eine Sigmoid-Aktivierungsfunktion einsetzt und der Decoder linear ist. (d) Nichtlinearer dreischichtiger Autoencoder, wobei sowohl Encoder als auch Decoder eine Sigmoid-Nichtlinearität verwenden. (Eigene Darstellung)}
