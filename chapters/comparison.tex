...
Decoders eine Matrix $\mat{W}_2 \in \real^{D \times d}$. Es kann nun gezeigt werden, dass sich die Zielfunktion eines solchen dreischichtigen Autoencoders mit linearen Aktivierungsfunktionen (LAE) auf die Form
\begin{equation}
...
Ladungsmatrix $\mat{A}$ auf. Dies gilt sogar dann, wenn im Encoder eines dreischichtigen
Autoencoders Sigmoid-Aktivierungsfunktionen verwendet werden \parencite[291, 293]{Bourlard.1988}. Jedoch unterscheidet sich die latente Repräsentation $\mat{Y}$
...
dreischichtiger Autoencoder die Richtungen der Hauptkomponenten lernen kann, muss ein
Regularisierungsterm zur Zielfunktion des Autoencoders hinzufügt werden, der die Größe der Gewichte
bestraft (siehe Regularisierung in \secref{ch:MethodenDerDimRed:ML:AE}).
...
dreischichtigen Autoencoders verglichen werden. Auf Bilddatensätzen können diese Matrizen so
umgeformt werden, dass sich für jede Zeile von $\mat{W}_1$ oder Spalte von $\mat{W}_2$ ein Bild
ergibt. \figref{fig:Gewichtsvergleich} setzt dieses Vorgehen auf dem FER-Datensatz um.
\begin{figure}[ht]
	\centering
...
\end{figure}
Hierbei ist in \captiona die Ladungsmatrix von PCA, in \captionb die Gewichtsmatrix $\mat{W}_2$ eines linearen dreischichtigen Autoencoders und in \captionc die ersten $d$ Links-Singulärvektoren der Decoder-Gewichtsmatrix $\mat{W}_2$ eines linearen Autoencoders mit \textit{weight decay} dargestellt. Die Gewichte des linearen Autoencoders zeigen bereits eine Ähnlichkeit zu der Ladungsmatrix $\mat{A}$ der Principal Component Analysis, jedoch sind einige Unterschiede erkennbar. Die Links-Singulärvektoren der Decoder-Gewichtsmatrix sind bis auf das Vorzeichen fast identisch zu der Ladungsmatrix $\mat{A}$.

\subsubsection{Hinzunahme von Nichtlinearität}

Bei der Hinzunahme von Nichtlinearität zum Autoencoder, gehen die beschriebenen Zusammenhänge
verloren. Dazu wurden mehrere dreischichtige Autoencoder mit unterschiedlichen
...
den MNIST-Datensatz berechnet und in \figref{fig:Autoencoder-Nichtlinearitaet} dargestellt. Für
Visualisierungszwecke wurde die intrinsische Dimension dabei auf zwei gesetzt.
\begin{figure}[ht]
	\centering
	\includegraphics{autoencoders-nonlinearity.pdf}
	\caption[Latente Repräsentationen von PCA und drei Autoencoder mit unterschiedlichen Aktivierungsfunktionen]{Dargestellt sind die latenten Repräsentationen $\mat{Y}$ einer \captiona Principal Component Analysis und mehrerer dreischichtiger Autoencoder mit \captionb linearen Aktivierungsfunktionen, \captionc Sigmoid-Aktivierung im Encoder und linearem Decoder und \captiond Sigmoid-Aktivierungen im Encoder und Decoder. Alle Autoencoder wurden mit \textit{weight decay} ($\lambda = \expnum{1}{-6}$) trainiert.}
...
Aktivierungsfunktionen eine sehr ähnliche latente Repräsentation wie die Principal Component
Analysis lernt. Eine Regularisierung der Gewichte begünstigt dabei Orthogonalität in der gefundenen
Lösung des Autoencoders. Damit kann für sehr große Datensätze ein alternativer Algorithmus mithilfe
des Autoencoders verwendet werden, um die Richtungen der Hauptkomponenten zu lernen. Dabei umgeht
man die Eigenwertszerlegung der $D \times D$-Kovarianzmatrix der Daten, indem nur die
