...
lokale Struktur erhalten wird. Eine $k$-Nachbarschaft $\set{M}_k(i)$ eines Punktes $\vect{x}_i$ ist
definiert als die Menge der $k$-nächsten Punkte zu $\vect{x}_i$ ($i = 1, \ldots, n$). Analog kann
die $k$-Nachbarschaft $\widetilde{\set{M}}_k(i)$ des dazugehörigen niedrigdimensionalen Punktes
$\vect{y}_i$ definiert werden. Diese Nachbarschaft wird in einer Dimensionsreduktion erhalten, wenn
$\set{M}_k(i) = \widetilde{\set{M}}_k(i)$, das heißt die Nachbarschaften bleiben von der
...
\begin{equation}
	T_k = 1 - \frac{2}{nk(2n - 3k - 1)} \sum_{i = 1}^{n}\sum_{j \in \set{U}_k(i) } \left( \tilde{r}(i, j) - k \right)
\end{equation}
...
$\vect{y}_i$ geordnet sind. Der Term vor der Summation skaliert das Qualitätskriterium so, dass $0
	\leq T_k \leq 1$ gilt.\footnote{Dies gilt nur für den Fall, dass $k < n/2$ gilt.} Ein Wert von $T_k
	= 1­$ spricht für eine hohe Vertrauenswürdigkeit.

Analog wird die Kontinuität $C_k$ über $\set{V}_k(i)$ wie folgt definiert:
\begin{equation}
	C_k = 1 - \frac{2}{nk(2n - 3k - 1)} \sum_{i = 1}^{n}\sum_{j \in \set{V}_k(i) } \left( r(i, j) - k \right)
\end{equation}
wobei $r(i, j)$ nun den Rang zwischen den Datenpunkten in der hochdimensionalen Repräsentation bezeichnet \parencite[487]{Venna.2001}. Auch hier gilt $0 \leq C_k \leq 1$ und höher ist besser. Die Kontinuität
